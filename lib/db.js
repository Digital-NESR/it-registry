...
import pg from "pg";
import { allKeys, fieldByKey, columnType, isArrayField, isObjField } from "./schema.js";
...
function isObjKey(key) {
  const f = fieldByKey[key];
  return f ? isObjField(f) : false;
}
...
function toColumnValue(key, val) {
  ...
  if (isObjKey(key)) {
    return JSON.stringify(val && typeof val === "object" ? val : {});
  }
...
export async function ensureSchema() {
  ...
  `);
  await query(`ALTER TABLE documents ADD COLUMN IF NOT EXISTS category TEXT`);
  ...
  `);
  await query(`ALTER TABLE audit_log ADD COLUMN IF NOT EXISTS ip TEXT`);
...
/** Set a user's role (creating the row if needed). An optional profile
 *  (from the directory) seeds display_name/job_title for a pre-provisioned
 *  user without ever overwriting values a real login has already filled in. */
export async function setRole(email, role, updatedBy, profile = {}) {
  if (!ROLE_VALUES.includes(role)) throw new Error("Invalid role");
  await query(
    `INSERT INTO user_roles (email, role, display_name, job_title, updated_by)
       VALUES ($1,$2,$3,$4,$5)
     ON CONFLICT (email) DO UPDATE SET
       role         = EXCLUDED.role,
       display_name = COALESCE(user_roles.display_name, EXCLUDED.display_name),
       job_title    = COALESCE(user_roles.job_title, EXCLUDED.job_title),
       updated_by   = EXCLUDED.updated_by,
       updated_at   = now()`,
    [email, role, profile.displayName || null, profile.jobTitle || null, updatedBy || null]
  );
}
...
/* ---------- audit log ---------- */
export async function logAudit({ actorEmail, actorName, action, entityType, entityId, summary, metadata, ip }) {
  try {
    await query(
      `INSERT INTO audit_log (actor_email, actor_name, action, entity_type, entity_id, summary, ip, metadata)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
      [actorEmail || null, actorName || null, action, entityType || null,
       entityId != null ? String(entityId) : null, summary || null, ip || null,
...
export async function listAudit({ limit = 100, offset = 0, action, actor } = {}) {
  ...
  const { rows } = await query(
    `SELECT id, ts, actor_email AS "actorEmail", actor_name AS "actorName", action,
            entity_type AS "entityType", entity_id AS "entityId", summary, ip, metadata
...
function rowToApp(row, docs = []) {
  ...
  for (const k of COLS) {
    ...
    else if (isObjKey(k)) v = v && typeof v === "object" ? v : {};
    app[k] = v;
  }
  app.documents = docs;
  return app;
...
async function docsByApp(ids) {
  ...
  const { rows } = await query(
    `SELECT id, application_id, filename, content_type, size, uploaded_by, category, uploaded_at
...
/* ---------- documents ---------- */
export async function insertDocument({ applicationId, filename, contentType, size, data, uploadedBy, category }) {
  const { rows } = await query(
    `INSERT INTO documents (application_id, filename, content_type, size, data, uploaded_by, category)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id, application_id, filename, content_type, size, uploaded_by, category, uploaded_at`,
    [applicationId, filename, contentType, size, data, uploadedBy, category || null]
  );
  return rows[0];
}
