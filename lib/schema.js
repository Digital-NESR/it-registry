...
export const refs = {
  ...
  cloudProvider: ["AWS", "Azure", "GCP", "Oracle Cloud", "IBM Cloud", "Other", "N/A"],
  ...
  yesNo: ["Yes", "No"],
  aiAccess: ["We call it via our own API", "Provider-managed (built into the tool)", "Hybrid"],
  aiBilling: ["Charged back per usage", "Included in tool cost", "No additional cost"],
  promptInjectionHandledBy: ["NESR (in-house)", "Service provider", "Both", "Not addressed"],
  ...
  monitoringTool: ["Datadog", "Dynatrace", "Splunk", "Nagios", "Azure Monitor", "Microsoft Sentinel", "Custom", "None"],
...
// Major AI providers (for the AI models dropdown).
export const AI_PROVIDERS = ["OpenAI", "Azure OpenAI", "Anthropic", "Google (Gemini / Vertex)",
  "AWS Bedrock", "Meta (Llama)", "Mistral", "Cohere", "Hugging Face", "IBM watsonx", "Other"];
...
export const domains = [
  { key: "identity", label: "Identity & Ownership", icon: "id", fields: [
    ...
    { key: "description", label: "Description", long: true, rows: 5, placeholder: "General description of the application, purpose and use case…" },
    { key: "subDomain", label: "Sub-Domain", placeholder: "Mention the URL or add URL…", hint: "The sub-domain / URL the app is served on, e.g. wellview.nesr.com." },
  ...
  { key: "technical", label: "Technical Profile", icon: "chip", fields: [
    ...
    { key: "cloudProvider", label: "Cloud Provider", ref: "cloudProvider", hint: "Cloud provider hosting the application. Choose 'Other' to specify a different one." },
    { key: "hostingService", label: "Hosting Service", showIf: { key: "cloudProvider", in: ["Other"] }, hint: "Free text — name the cloud provider / hosting service (shown because Cloud Provider is 'Other')." },
  ...
  { key: "risk", label: "Risk & Compliance", icon: "shield", fields: [
    ...
    { key: "certifications", label: "Certifications", multi: true, ref: "certifications", certDetails: true, hint: "Certifications held by the app / vendor. Each selected one gets its own attachment + expiry below." },
    { key: "certExpiry", label: "Certification Expiry", obj: true, hidden: true }, // { "<cert>": "YYYY-MM-DD" }
  ...
  { key: "ai", label: "AI / GenAI", icon: "chip", fields: [
    { key: "aiUsed", label: "This application uses AI / GenAI", checkbox: true, hint: "Tick if the application uses AI or generative-AI capabilities." },
    { key: "aiAccess", label: "How AI is accessed", ref: "aiAccess", showIf: { key: "aiUsed", in: ["Yes"] }, hint: "Do we call the model via our own API, or does the provider manage it inside the tool?" },
    { key: "aiBilling", label: "AI cost model", ref: "aiBilling", showIf: { key: "aiUsed", in: ["Yes"] }, hint: "Is AI usage charged back to us, or included in the tool's cost?" },
    { key: "aiCostCapped", label: "Usage cap / limit in place?", ref: "yesNo", showIf: { key: "aiBilling", in: ["Charged back per usage"] }, hint: "If charged per usage, is there a cap to control spend?" },
    { key: "aiCapDetails", label: "Cap / limit details", showIf: { key: "aiCostCapped", in: ["Yes"] }, hint: "e.g. monthly token or spend cap, rate limits." },
    { key: "promptInjectionHandledBy", label: "Prompt-injection protection handled by", ref: "promptInjectionHandledBy", showIf: { key: "aiUsed", in: ["Yes"] }, hint: "Who detects and mitigates prompt injection & misuse?" },
    { key: "promptInjectionControls", label: "Prompt-injection controls", long: true, showIf: { key: "aiUsed", in: ["Yes"] }, hint: "How prompt injection / misuse is detected and mitigated (guardrails, input & output filtering, allow-lists, etc.)." },
    { key: "aiModels", label: "AI providers & models", repeater: true, addLabel: "Add model",
      columns: [
        { key: "provider", label: "Provider", type: "select", options: AI_PROVIDERS },
        { key: "model", label: "Model", type: "text" },
        { key: "purpose", label: "Used for", type: "text" },
      ],
      showIf: { key: "aiUsed", in: ["Yes"] }, hint: "List each AI provider/model in use and what it's used for." },
  ] },
  { key: "financial", label: "Financial & Licensing", icon: "coin", fields: [
    ...
    { key: "tco", label: "TCO ($)", money: true, computed: true, hint: "Auto-calculated: Annual License Cost + Annual Maintenance Cost." },
  ...
  { key: "vendor", label: "Vendor & SLA", icon: "building", fields: [
    { key: "appVendor", label: "Software Provider", hint: "The software provider / vendor of the application (leave blank if built in-house)." },
    { key: "isPartner", label: "Engaged via a partner (not the direct provider)", checkbox: true, hint: "Tick if a partner / reseller manages this rather than the direct provider." },
    { key: "providerContacts", label: "Software Provider Contacts", contacts: true, hint: "Add one or more contacts — name, email and phone for each." },
    { key: "partnerName", label: "Partner Name", showIf: { key: "isPartner", in: ["Yes"] }, hint: "Name of the partner / reseller." },
    { key: "partnerContacts", label: "Partner Contacts", contacts: true, showIf: { key: "isPartner", in: ["Yes"] }, hint: "Add one or more partner contacts — name, email and phone for each." },
    { key: "vendorSupportTimings", label: "Support Timings", hint: "Support hours, e.g. 24/7, Business hours GST." },
    { key: "vendorSla", label: "Vendor SLA", hint: "Service level committed (response/resolution)." },
  ...
  { key: "value", label: "Business Value & Criticality", icon: "star", fields: [
    ...
    { key: "appType", label: "Application Type", toggle: true, options: ["Support", "Operation"], hint: "Is this a Support or an Operation application?" },
  ...
  { key: "dependencies", label: "Dependencies & Integrations", icon: "link", fields: [
    ...
    { key: "dataFlowDoc", label: "Data Flow Doc", file: true, hint: "Upload the data-flow diagram / document." },
...
export const isArrayField = (f) => !!(f.multi || f.apps || f.contacts || f.repeater);
export const isObjField = (f) => !!f.obj;
export const isJsonField = (f) => isArrayField(f) || isObjField(f);
