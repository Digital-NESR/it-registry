...
/* shared CSV export (used by Registry too) */
function exportCSV(rows, filename) {
  const cols = [{ key: "appId", label: "App ID" }, { key: "name", label: "App Name" }];
  window.NESR.domains.forEach(d => d.fields.forEach(f => { if (f.key !== "appId") cols.push(f); }));
...
/* ---------- card wrapper ---------- */
function Panel({ title, sub, children, action, span, pad = 18 }) {
  return (
  ...
  return (
    <div style={{ padding: "22px 26px 40px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18, flexWrap: "wrap" }}>
        ...
        <Dropdown label="Department" value={filters.department} options={window.NESR.departments}
          onChange={v => setFilters(f => ({ ...f, department: v }))} />
        <Dropdown label="Status" value={filters.status} options={window.NESR.refs.status}
          onChange={v => setFilters(f => ({ ...f, status: v }))} />
        <Dropdown label="Criticality" value={filters.businessCriticality} options={window.NESR.refs.businessCriticality}
        ...
        <button onClick={() => { exportCSV(filtered, `NESR-registry-${window.NESR.today}.csv`); push(`Exported ${filtered.length} applications to CSV`); }}
          style={primaryBtn}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(12, 1fr)", gap: 16 }}>
        ...
        <Panel title="Business criticality" sub="Share of portfolio by tier" span={6}>
          <SegBar data={byCrit} colorMap={critColorByLabel} onSeg={l => {
            const full = window.NESR.refs.businessCriticality.find(c => c.startsWith(l)); toggle("businessCriticality", full);
...
const ddItem = (active) => ({ display: "block", width: "100%", textAlign: "left", padding: "7px 10px", borderRadius: 7,
  border: "none", background: active ? "var(--green-100)" : "transparent", color: active ? "var(--green-700)" : "var(--text)",
  fontSize: 12.5, fontWeight: active ? 600 : 500 });

const primaryBtn = { display: "inline-flex", alignItems: "center", gap: 7, padding: "8px 14px", borderRadius: 8,
  border: "none", background: "var(--green-600)", color: "#fff", fontSize: 12.5, fontWeight: 600, boxShadow: "var(--shadow-sm)" };
const textBtn = { display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 9px", borderRadius: 7,
  border: "none", background: "transparent", color: "var(--text-soft)", fontSize: 12.5, fontWeight: 600 };

Object.assign(window, { Dashboard, exportCSV, Dropdown, Panel, primaryBtn, textBtn, ddItem });
