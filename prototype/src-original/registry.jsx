  ...
  name: { label: "Application", w: 230, sticky: true, render: (a) => (
    <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
      <span style={{ width: 30, height: 30, borderRadius: 7, background: "var(--green-100)", color: "var(--green-700)",
        display: "grid", placeItems: "center", fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{a.alias.slice(0, 3)}</span>
  ...
  return (
    <div style={{ padding: "20px 26px 10px", height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 13, flexWrap: "wrap" }}>
        ...
        <Dropdown label="Status" value={filters.status} options={window.NESR.refs.status} onChange={v => setFilters(f => ({ ...f, status: v }))} />
        <Dropdown label="Department" value={filters.department} options={window.NESR.departments} onChange={v => setFilters(f => ({ ...f, department: v }))} />
        <Dropdown label="Criticality" value={filters.businessCriticality} options={window.NESR.refs.businessCriticality} onChange={v => setFilters(f => ({ ...f, businessCriticality: v }))} />
        ...
        <button onClick={() => { exportCSV(rows, `NESR-registry-${window.NESR.today}.csv`); push(`Exported ${rows.length} applications to CSV`); }} style={primaryBtn}>
...
const ghostBtn = { display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 11px", borderRadius: 8,
  border: "1px solid var(--line-strong)", background: "var(--surface)", color: "var(--text)", fontSize: 12.5, fontWeight: 600 };

Object.assign(window, { Registry, ghostBtn });
