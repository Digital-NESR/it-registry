...
function Approvals() {
  const { apps, role, decide, push, goDetail, setView } = useStore();
  ...
  const approve = (a) => { decide(a.id, "Approved", ""); push(`${a.name} approved & added to the registry`); };
  const doReject = () => { decide(reject.id, "Rejected", note); push(`${reject.name} rejected`, "err"); setReject(null); setNote(""); };
            ...
            <div key={a.id} style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 14, padding: 18,
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 15, flexWrap: "wrap" }}>
                <div style={{ width: 44, height: 44, borderRadius: 11, background: "var(--green-100)", color: "var(--green-700)",
                  display: "grid", placeItems: "center", fontSize: 14, fontWeight: 700, flexShrink: 0 }}>{a.alias.slice(0, 3)}</div>
