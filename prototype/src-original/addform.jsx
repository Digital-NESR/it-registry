...
function emptyApp() {
  const o = { name: "" };
  window.NESR.domains.forEach(d => d.fields.forEach(f => { o[f.key] = ""; }));
  return o;
}
  ...
  if (f.ref) {
    input = (
      <select {...common}>
        ...
        {window.NESR.refs[f.ref].map(o => <option key={o} value={o}>{o}</option>)}
      </select>
    );
  } else if (f.long) {
...
function AddForm() {
  const { submitApp, push, setView, goDetail, role, prefill, setPrefill, apps } = useStore();
  const [step, setStep] = useState(0);
  const [data, setData] = useState(() => prefill ? { ...prefill } : emptyApp());
  const [touched, setTouched] = useState(false);
  const editing = !!prefill;

  useEffect(() => () => setPrefill(null), []); // clear prefill on unmount

  const set = (k, v) => setData(d => ({ ...d, [k]: v }));
  const errors = {};
  REQUIRED.forEach(k => { if (!String(data[k] || "").trim()) errors[k] = "Required"; });
  const stepDomains = STEPS[step].domains;
  const stepHasError = stepDomains.some(dk => window.NESR.domains.find(d => d.key === dk).fields.some(f => errors[f.key]))
  ...
  const finalize = (asDraft) => {
    setTouched(true);
    if (Object.keys(errors).length && !asDraft) { push("Please complete all required fields", "err"); 
      // jump to first step with an error
      for (let i = 0; i < STEPS.length; i++) { const ds = STEPS[i].domains; if ((STEPS[i].lead && errors.name) || ds.some(dk => window.NESR.domains.find(d=>d.key===dk).fields.some(f=>errors[f.key]))) { setStep(i); break; } }
      return;
    }
    submitApp(data, asDraft);
    ...
    <div style={{ padding: "20px 26px 60px", maxWidth: 920, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ margin: "4px 0 0", fontSize: 13, color: "var(--text-soft)" }}>
          New applications are submitted to the Head of IT ({window.NESR.headOfIT}) for approval before joining the registry.
        </p>
      </div>
      ...
      <div style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 16, padding: 24, boxShadow: "var(--shadow-sm)", minHeight: 340 }}>
        {step < STEPS.length - 1 ? (
          <div key={step} className="view-enter">
            ...
            {stepDomains.map(dk => {
              const dom = window.NESR.domains.find(d => d.key === dk);
              return (
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 11, marginTop: 18 }}>
        ...
        <div style={{ flex: 1 }} />
        <button onClick={() => finalize(true)} style={{ ...ghostBtn }}><Icon name="doc" size={14} /> Save as draft</button>
        {step < STEPS.length - 1
          ? <button onClick={next} style={primaryBtn}>Continue <Icon name="chevRight" size={14} /></button>
          : <button onClick={() => finalize(false)} style={primaryBtn}><Icon name="check" size={15} /> Submit for approval</button>}
      </div>
    </div>
  );
}
